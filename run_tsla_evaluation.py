"""
run_tsla_evaluation.py

Runs the same detection and forecasting evaluations already validated
for NVDA and AMD, on TSLA, using its just-finalized earnings ground
truth (data/earnings_dates_FINAL_tsla.csv).

Note on ground truth provenance: of the 35 confirmed TRUE dates, 32
were confirmed by cross-referencing Tesla's known quarterly
announcement-date history (not all individually re-read from the raw
8-K Item text this session) and 3 were directly verified against the
8-K's actual Item classification. See
data/earnings_candidates_FOR_REVIEW_tsla.csv's reviewer_note column
for per-row provenance if this needs auditing later.

Run from the repo root:  python run_tsla_evaluation.py
"""
...
TSLA_EARNINGS_CSV = "data/earnings_dates_FINAL_tsla.csv"
...
def run_detection_sweep():
    print_header("TSLA -- Detection tolerance sweep (KSWIN, one-sided) + chance baseline")
    ...
    for tol in [10, 15, 20, 25, 30]:
        results = run_full_rolling_evaluation(
            ticker="TSLA", tolerance=tol, one_sided=True,
            earnings_csv_path=TSLA_EARNINGS_CSV,
        )
    ...
    print("\nADWIN / Page-Hinkley alarm counts (sanity check -- third ticker, same zero-alarm pattern?):")
    for detector in ["ADWIN", "Page-Hinkley"]:
        results = run_full_rolling_evaluation(
            ticker="TSLA", tolerance=20, one_sided=True,
            earnings_csv_path=TSLA_EARNINGS_CSV,
        )
...
def run_forecasting_comparison(df):
    print_header("TSLA -- Single-window retraining comparison (400-row eval window) + baselines")
