...
# Sliding window: each retrain uses only the most recent 600 rows,
# not all history -- this tests whether the "never beats everything"
# result was caused by retrains being diluted by a large, mostly
# unchanged expanding training set.
TRAIN_WINDOW_SIZE = 600

print(f"Total rows: {len(df)}, initial_train_size: {initial_train_size}, "
      f"evaluating last {EVAL_WINDOW} rows, train_window_size={TRAIN_WINDOW_SIZE}")
